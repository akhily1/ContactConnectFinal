...
- (void)doOauthWithCode: (NSString *)code {
    ...
    NSString *postString =[NSString stringWithFormat:@"client_id=%@&client_secret=%@&grant_type=authorization_code&redirect_uri=%@&code=%@",CLIENT_ID,CLIENT_SECRET,REDIRECT_URI,SC_CODE];
...
-(NSMutableArray *) searchForTracksWithQuery: (NSString *) query {
    ...
    NSString *jsonString =[NSString stringWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/users.json?oauth_token=%@&client_id=%@&q=%@",SC_API_URL,self.scToken,CLIENT_ID,query]] encoding:NSUTF8StringEncoding error:nil];
    NSLog(@"%@",jsonString1);
    NSMutableArray *musicArray =[jsonString objectFromJSONString];
    self.scTrackResultList = [[NSMutableArray alloc]init];
